"""Canonical implementation of the Phase 17 acceptance harness."""
...
def _build_extended_context_events(ohlcv_frame: pl.DataFrame) -> list[SignalEvent]:
    ...
    for index, timestamp in enumerate(timestamps):
        ...
        events.extend(
            [
                SignalEvent(source="okx_funding", signal_type="funding_rate", symbol=DEFAULT_PHASE17_SYMBOL, timestamp=timestamp, value=funding_rate, metadata={"fixture": "phase17"}),
                SignalEvent(source="okx_orderbook", signal_type="spread_bps", symbol=DEFAULT_PHASE17_SYMBOL, timestamp=timestamp, value=spread_bps, metadata={"fixture": "phase17"}),
                SignalEvent(source="okx_orderbook", signal_type="depth_imbalance", symbol=DEFAULT_PHASE17_SYMBOL, timestamp=timestamp, value=depth_imbalance, metadata={"fixture": "phase17"}),
                SignalEvent(source="liquidations", signal_type="liquidation_imbalance", symbol=DEFAULT_PHASE17_SYMBOL, timestamp=timestamp, value=liquidation_imbalance, metadata={"fixture": "phase17"}),
                SignalEvent(source="open_interest", signal_type="open_interest", symbol=DEFAULT_PHASE17_SYMBOL, timestamp=timestamp, value=open_interest, metadata={"fixture": "phase17"}),
            ]
        )
    return events
...
def _annotate_context_frame(context_frame: pl.DataFrame, query: AcceptanceQuery) -> pl.DataFrame:
    if context_frame.is_empty():
        return pl.DataFrame({"timestamp": [], "symbol": [], "context_tags": [], "required_context_tags_pass": []})
...
def _attach_context_frame(frame: pl.DataFrame, context_frame: pl.DataFrame) -> pl.DataFrame:
    ...
    rename_map = {column: ("context_timestamp" if column == "timestamp" else f"context_{column}") for column in context_frame.columns}
    ...
    return _sanitize(
        {
            ...
            "metrics": metrics,
            "validation": {"passed": validation.passed, "reasons": validation.reasons},
        }
    )
    ...
    return _sanitize(
        {
            ...
            "metric_deltas": _metric_deltas(baseline_summary.get("metrics", {}), candidate_summary.get("metrics", {})),
...
def _frame_payload(frame: pl.DataFrame) -> dict[str, Any]:
    return _sanitize({"schema": {column: str(dtype) for column, dtype in frame.schema.items()}, "rows": frame.sort(frame.columns[0]).to_dicts() if frame.columns else []})
...
__all__ = ["AcceptanceQuery", "load_acceptance_query", "main", "parse_args", "run_acceptance"]

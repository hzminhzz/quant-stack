...
DATA_PATH = Path("/root/quant-factory/data/ohlcv/bybit/linear/1m/BTCUSDT.parquet")
OUT_DIR = Path("artifacts/smart_dca_btc")
...
def main():
    ...
    # Check what timestamp looks like and filter it to 2022-2025
    time_col_raw = "timestamp" if "timestamp" in df.columns else "time" if "time" in df.columns else "open_time" if "open_time" in df.columns else df.columns[0]
    
    # Cast timestamp to datetime if it's integer (milliseconds usually)
    if df.schema[time_col_raw] in [pl.Int64, pl.Float64]:
        df = df.with_columns(pl.from_epoch(pl.col(time_col_raw), time_unit="ms").alias(time_col_raw))
    
    df = df.filter(
        (pl.col(time_col_raw).dt.year() >= 2022) & (pl.col(time_col_raw).dt.year() <= 2025)
    )
    ...
    params = SmartDCAParams(
        use_bid_as_avr=True,
        contract_size=1.0,  # Standard lot for BTC is 1 BTC
